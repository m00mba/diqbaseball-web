              ...
              <div className={styles.section}>
                ...
                {aiReport && (
                  <div className={styles.aiReport}>
                    ...
                    {(() => {
                      const strengthsMatch = aiReport.match(/STRENGTHS:([\s\S]*?)(?=OPPORTUNITIES:|$)/i)
                      const opportunitiesMatch = aiReport.match(/OPPORTUNITIES:([\s\S]*?)(?=RECOMMENDED DRILLS:|$)/i)
                      const drillsMatch = aiReport.match(/RECOMMENDED DRILLS:([\s\S]*?)$/i)
                        ...
                        <>
                          {strengthsMatch && (
                            <div style={{ marginBottom: 14 }}>
                              ...
                              <p style={{ margin: 0, fontSize: 13, lineHeight: 1.6 }}>{strengthsMatch[1].trim()}</p>
                            </div>
                          )}
                          {opportunitiesMatch && (
                            <div style={{ marginBottom: 14, paddingTop: 14, borderTop: '1px solid #e5e5e5' }}>
                              ...
                              <p style={{ margin: 0, fontSize: 13, lineHeight: 1.6 }}>{opportunitiesMatch[1].trim()}</p>
                            </div>
                          )}
                          {drillsMatch && (
                            <div style={{ paddingTop: 14, borderTop: '1px solid #e5e5e5' }}>
                              ...
                              {drillsMatch[1].trim().split('\n').filter(l => l.trim()).map((line, j) => (
                                <p key={j} style={{ margin: '0 0 6px 0', fontSize: 13, lineHeight: 1.6 }}>{line.trim()}</p>
                              ))}
                            </div>
                          )}
                        </>
